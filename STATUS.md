...
**Current source delta:** after PR #21, the source additionally removes the clipped
launcher blur, makes the nonactivating launcher key on demand for its first click, makes the card
key and pointer-ready on reveal, queues a visible refresh behind in-flight launch loading, keeps
Focus/Important interaction available during snapshot reads, and scopes double-click Open to the
NOW card. The receipts below describe the verified local checkout; hosted proof remains
commit-scoped and must be read from the matching PR or CI run.
...
| --- | --- | --- |
| Focus surface | The floating launcher, Dock reopen, and menu-bar item lead to the compact focus card; Organizer is explicit. The card is pointer-ready on reveal, and snapshot refresh no longer disables retained rows. A quick task click remains Open; a stationary hold selects the row, and continuing the same press lifts its visible preview for full-row drag. Double-click Open is limited to the NOW card. Each successful drop remains one atomic `move-before` mutation. | The local working candidate passed the isolated pointer journey; physical human-pointer and VoiceOver journeys remain separate evidence. |
...
| Launcher reopen | The real 60×60 macOS launcher handles clicks directly in AppKit, prewarms the card, and queues a visible refresh after reveal. Its rounded material and border no longer cast an outer blur that can clip to the panel rectangle. Stationary click, 2-pixel click, move/edit recovery, accessibility press, outer-edge, inactive-first-hold, and exact NOW double-open automation pass within the 200 ms budget. | This is isolated local automation on the current host, not every physical pointer, machine-load profile, or VoiceOver journey. |
