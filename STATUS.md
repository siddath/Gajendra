...
- Compact, single-owner card scrolling; expandable provider-wide Running rows; fixed search footers in both the focus card and full organizer; bottom-edge queue overflow shortcuts; whole-surface hover feedback; aligned Open → Running/Ready → Provider NOW actions; and keyboard-capable multi-term search whose entire capsule focuses on the first cross-app click, preserves normal multi-character input, and selects an existing query once per new focus entry. The organizer remains resizable, with Dock/menu-bar recovery and a reversible login item.
...
- Installed app: visible name `Gaja`, compatibility executable `Gajendra`, version 0.3.1, ad-hoc signature valid; installed and source-built binary SHA-256 `14e7959468f13eead0f8149d8ffcb88c9179de1ee1f9aa9d7eca63168e8f3359`; installed ICNS SHA-256 `ce14ad09bdc2dd48a48fa9a39c9407b356571ae207bdfa0bbaba8ec040b8faff`; bundled service SHA-256 `e37e1714d5912db9992a54573d0475398622e093337573eace2fb067476ef2d1`.
...
- Running installed runtime: relaunched as PID `91066`, with the persisted launcher at Bottom Right. A clean one-second hover left only the 60 × 60 pill visible; primary click opened the 660 × 610 detail card for the current Compact preference, and a second click closed it. Installed Accessibility proof covered the real search field, changing result counts, explicit no-match state, retained focus after Clear, and the matching Organizer field. The immediately previous installed app is recoverable at `/Users/i556940/Applications/Gajendra.app.rollback-search-fix-20260816-000335`.
