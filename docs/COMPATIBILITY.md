...
| First-launch source setup | One skippable local-source scan on a clean native launch; replayable from Settings and the application menu; existing native users migrate without an unsolicited popup | Isolated fresh/upgrade `UserDefaults` self-tests, native light/dark previews, source validator, and installed replay proof | Organizer source chips and application-menu recovery remain available |
